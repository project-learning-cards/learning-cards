...
export const getCardsList =
  (params: GetCardsAPIParamsType): AppThunkType =>
  async (dispatch: Dispatch<ActionCardsListType>) => {
    try {
...
export const addCard =
  (data: AddCardDataType): AppThunkType =>
  async (dispatch: Dispatch<ActionCardsListType>) => {
    try {
      await CardsListAPI.addCard(data);
...
export const deleteCard =
  (params: { id: string; cardPack_id: string }): AppThunkType =>
  async (dispatch: Dispatch<ActionCardsListType>) => {
    try {
      await CardsListAPI.deleteCard(params);
...
export const gradeCardTC = (grade: number, card_id: string) => async () => {
  try {
