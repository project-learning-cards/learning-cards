...
export type ActionPacksListType = StartPackListLoadingAT | SuccessPackListLoadingAT | LoadingErrorAT | setPageNumberAT;
...
export const LoadingErrorAC = (error: string) => ({ type: 'packList/LOADING-ERROR', error } as const);
...
export const updatePackListTC =
  (params: GetPacksListParamsType): AppThunkType =>
...
export const addPack =
  (data: AddCardsAPIParamsType, user_id: string | undefined): AppThunkType =>
  async (dispatch: Dispatch<ActionPacksListType>, getState: GetAppStateType) => {
    const { sortPacks, pageCount, packName, min, max, page } = getState().packsList.packsParams;
    try {
...
export const deletePack =
  (params: { id: string }, _id: string): AppThunkType =>
  async (dispatch: Dispatch<ActionPacksListType>, getState: GetAppStateType) => {
    const { sortPacks, pageCount, packName, min, max, page } = getState().packsList.packsParams;
    try {
